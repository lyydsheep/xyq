...
func (r *authRepository) StoreRefreshToken(ctx context.Context, userID int64, refreshToken string, expiresAt time.Time) error {
	...
	if err != nil {
		r.logger.WithContext(ctx).Errorf("Failed to store refresh token for user_id: %d, error: %v", userID, err)
		return err
	}
...
func (r *authRepository) GetUserIDByRefreshToken(ctx context.Context, refreshToken string) (int64, error) {
	...
	if err != nil {
		...
		r.logger.WithContext(ctx).Errorf("Failed to get refresh token, error: %v", err)
		return 0, err
	}
...
func (r *authRepository) DeleteRefreshToken(ctx context.Context, refreshToken string) error {
	...
	if err != nil {
		r.logger.WithContext(ctx).Errorf("Failed to delete refresh token, error: %v", err)
		return err
	}
...
func (r *authRepository) DeleteAllRefreshTokens(ctx context.Context, userID int64) error {
	...
	if err := iter.Err(); err != nil {
		r.logger.WithContext(ctx).Errorf("Failed to scan refresh tokens for user_id: %d, error: %v", userID, err)
		return err
	}
	...
	if len(keys) > 0 {
		...
		if err != nil {
			r.logger.WithContext(ctx).Errorf("Failed to delete refresh tokens for user_id: %d, error: %v", userID, err)
			return err
		}
...
func (r *authRepository) RefreshTokenAtomically(ctx context.Context, userID int64, oldToken, newToken string, expiresAt time.Time) error {
	...
	_, err := pipe.Exec(ctx)
	if err != nil {
		r.logger.WithContext(ctx).Errorf("Failed to refresh token atomically for user_id: %d, error: %v", userID, err)
		return err
	}
